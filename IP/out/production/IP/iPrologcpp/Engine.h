...
//ORIGINAL LINE: private final ArrayList<String> slist;
  ...
  private:
	  static std::vector<int> toNums(std::vector<Clause*> &clauses) final;

	  /*
	   * encodes string constants into symbols while leaving
	   * other data types untouched
	   */
	  int encode(int const t, const std::wstring &s);
  ...
  private:
	   ...
	   */
  public:
	  virtual std::wstring showTerm(int const x);
	   ...
	   */
	  virtual std::any exportTerm(int x);
	   ...
	   */
	  std::wstring showCell(int const w);

	  /**
	   * a displayer for cells
	   */

	  virtual std::wstring showCells(int const __super, int const len);

	  virtual std::wstring showCells(std::vector<int> &cs);
