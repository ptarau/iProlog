	  ...
	  switch (c)
	  {
		case TT_WORD:
		{
		  ...
		  else
		  {
			try
			{
			  constexpr int n = static_cast<int>(sval);
			  if (std::abs(n) < 1 << 28)
			  {
				t = L"n:" + sval;
			  }
...
//ORIGINAL LINE: final ArrayList<ArrayList<ArrayList<String>>> Wsss = new ArrayList<ArrayList<ArrayList<String>>>();
	  std::vector<std::vector<std::vector<std::wstring>>> Wsss;
	...
	std::wstring Toks::toString(std::vector<std::any> &Wsss)
	{
